...
from imods.models.constants import BillingType, AccountStatus, ItemStatus, UserRole
from imods.models import User, Order, Item, Device, Category, BillingInfo, VIPUsers
from imods.models import UserRole, OrderStatus, Review, WishList, Banner
from imods.decorators import require_login, require_json, require_beta_access
...
@api_mod.route("/download/<path:path>")
def download_package(path):
    bucket = app.config["S3_PKG_BUCKET"]
    url = "https://" + bucket + ".s3.amazonaws.com/" + path + "?" + request.query_string;
    ...
    package = paths[1]
    item = Item.query.filter_by(pkg_name=package).first()
    if not item:
...
#curl -F file=@/Users/marcusferrario/Downloads/segment_export_5acfe1e42e.csv https://developer.imods.co/api/admin/vip/upload

@api_mod.route("/admin/vip/upload", methods=["POST"])
...
def vip_csv():
    ...
    with db_scoped_session() as se:
        ...
        for line in lines:
            vipstatus = VIPUsers.query.get(line.lower())
            if not vipstatus:
                newvip = VIPUsers(vip_email=line.lower())
                try:
                    se.add(newvip)
                    se.commit()
                except:
...
def user_register():
    ...
    *** Response ***
    ...
    :status 200: no error :py:obj:`.success_response`
    :status 400: :py:exc:`.BadJSONData`
    :status 409: :py:exc:`.UserAlreadyRegistered`
    """
    ...
    vipstatus = VIPUsers.query.get(req["email"].lower())
    if not vipstatus:
        raise UserSignupMaxReached()
...
@api_mod.route("/item/list", defaults={"iid": None, "pkg_names": None, "cat_names": None, "author_identifier": None, "search_term": None, "pkg_name": None})
@api_mod.route("/item/id/<int:iid>", defaults={"pkg_names": None, "cat_names": None, "author_identifier": None, "search_term": None, "pkg_name": None})
@api_mod.route("/item/pkgs/<pkg_names>", defaults={"iid": None, "cat_names": None, "author_identifier": None, "search_term": None, "pkg_name": None})
@api_mod.route("/item/pkg/<pkg_name>", defaults={"iid": None, "cat_names": None, "author_identifier": None, "search_term": None, "pkg_names": None})
@api_mod.route("/item/cat/<cat_names>", defaults={"iid": None, "pkg_names": None, "author_identifier": None, "search_term": None, "pkg_name": None})
@api_mod.route("/item/author/<author_identifier>", defaults={"iid": None, "pkg_names": None, "cat_names": None, "search_term": None, "pkg_name": None})
@api_mod.route("/item/search/<search_term>", defaults={"iid": None, "pkg_names": None, "cat_names": None, "author_identifier": None, "pkg_name": None})
@require_login
@require_json(request=False)
def item_list(iid, pkg_names, pkg_name, cat_names, author_identifier, search_term):
    """
    ...
    *** Response ***
    ...
    if iid is not None:
        ...
        pkg_query = urllib.unquote(pkg_names).decode('utf8') 
        pkg_names = pkg_query.lower().split(',')
        items = Item.query.filter(func.lower(Item.pkg_name).in_(pkg_names)).all()
    ...
    elif author_identifier is not None:
        items = Item.query.filter_by(author_id=author_identifier).all()
        author = User.query.filter_by(author_identifier=author_identifier).first()
        if not (items and author):
            raise ResourceIDNotFound
        user = User.query.get(author.uid)
        if not user:
            raise ResourceIDNotFound()
        expire = app.config["DOWNLOAD_URL_EXPIRES_IN"]
        s3_key = app.s3_assets_bucket.get_key(user.profile_image_s3_keypath)
        if s3_key:
            ...
            url = None
        get_public = operator.methodcaller('get_public')
        return dict(profile_image_url=url, fullname=author.fullname, twitter=author.twitter, contact_email=author.secondary_email, summary=author.summary, items=map(get_public, items))
    elif search_term is not None:
        #author_id support?
        items = Item.query.filter(and_(
            Item.status == ItemStatus.Approved,
            or_(
                Item.display_name.like('%'+search_term+'%'), 
        ...
        page = request.args.get("page") or 0
        items = Item.query
        items.filter_by(status=ItemStatus.Approved)
...
def item_add():
    ...
    *** Response ***
    ...
    item = Item(pkg_name=req['pkg_name'],
                ...
                pkg_conflicts=req.get('pkg_conflicts'),
                pkg_predepends=req.get('pkg_predepends'))
    with db_scoped_session() as se:
        se.add(item)
        se.commit()
        return item.get_public()
...
def get_item_assets(item, res_type):
    ...
    if res_type == "deb":
        ...
        result = deb
    #TODO: make local-url rather than s3
    ...
    *** Response ***
    ...
    with db_scoped_session() as ses:
        ...
        if item_ids:
            ...
            raise InternalException("This shouldn't happen")
